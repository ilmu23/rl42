...
/*   Updated: 2024/10/02 15:16:47 by ivalimak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#define getfn(x)		(set(x), key = ft_rl_getkey(), f = ft_rl_getmap(key), unset(x))
...
uint8_t	ft_rl_hlc(rl_input_t *input)
{
	...
	_preview();
	for (getfn(_HIDE_CURSOR); f != ft_rl_hl_acc; getfn(_HIDE_CURSOR))
...
uint8_t	ft_rl_hl_clr(__UNUSED rl_input_t *input)
{
	...
	for (i = 0, getfn(0); f != ft_rl_hl_acc; getfn(0))
	{
		if (f == ft_rl_ins)
		{
			if (i > 2)
